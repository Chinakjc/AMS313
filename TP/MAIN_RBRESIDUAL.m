...
Ns = [4,8,12,16];%[4,8,12,16,20,24,28,32];
...
for N = Ns
    ...
    for mu = mu_list'  
        ...
        residu2 = RB_compute_resnorm2(mu,Xrb,Respart_ll,Respart_la,Respart_aa);
    ...
    colorbar; 
    title(sprintf("log10 (residus/errors) with n = %d",N));
    figure
    scatter(mu_list(:,1), mu_list(:,2), 100, log10(residus), 'filled'); 
    colorbar; 
    title(sprintf("log10 (residus) with n = %d",N));
    figure
    scatter(mu_list(:,1), mu_list(:,2), 100, log10(errors), 'filled'); 
    colorbar; 
    title(sprintf("log10 (errors) with n = %d",N));
